        ...
        if missing_count > 0:
            ...
            if strategy == 'leave_nan':
                ...
                if 'subject_id' in imputed_df.columns and 'timepoint' in imputed_df.columns:
                    ...
                    remaining_nan = imputed_df[field].isnull().sum()
                    filled_count = missing_count - remaining_nan
                    print(f"    ⏭️ Filled {filled_count} values with longitudinal data")
                    
            elif strategy == 'smart':
                if missing_percent > 60:
                    print(f"    ⚠️ Too much missing data ({missing_percent:.1f}%) - keeping as NaN")
...
if __name__ == '__main__':
    ...
    print(f"📊 Excel support: {'✅' if EXCEL_SUPPORT else '❌'}")
